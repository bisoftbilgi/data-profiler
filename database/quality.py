...
def get_available_tests(column_info):
    ...
    tests = {
        ...
        'no_letters': {
            ...
            'available_for': ['varchar', 'char', 'nvarchar', 'nchar', 'text','decimal', 'numeric', 'float', 'double', 'int', 'bigint', 'smallint', 'tinyint']
        },
        'allowed_values': {
...
def run_quality_tests(connector, schema: str, table: str, selected_columns, selected_tests, custom_test_params=None):
    ...
    try:
        ...
        if sample_data and hasattr(sample_data[0], '__class__') and sample_data[0].__class__.__name__ == 'Row':
            sample_data = [tuple(row) for row in sample_data]
        # Flatten if needed (in case of nested tuples)
        if sample_data and len(sample_data[0]) == 1 and isinstance(sample_data[0][0], tuple):
            sample_data = [row[0] for row in sample_data]
...
def show_quality_tests_page(connector, schema: str):
    ...
    if selected_table:
        ...
        if selected_columns:
            ...
            # Group tests by type
            test_groups = {
                'Basic Checks': ['null_check', 'distinct_check', 'value_equality', 'column_correlation','no_numbers', 'no_letters'],
                'Numeric Checks': [
                    'range_check', 'positive_value', 'integer_type', 'zscore_outlier', 'eng_numeric_format', 'tr_numeric_format'
                ],
                'String Checks': [
                    'length_check', 'must_contain_at',  'allowed_values', 'length_between',
